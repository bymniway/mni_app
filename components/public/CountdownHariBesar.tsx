...
export default function CountdownHariBesar({ isEditor }: any = {}) {
  ...
  // STATE BARU: Menampung nilai penyesuaian Hijriah & Deteksi Maghrib
  const [hijriAdj, setHijriAdj] = useState(0);
  const [isAfterMaghrib, setIsAfterMaghrib] = useState(false);

  // 1. FETCH HIJRI ADJ & JADWAL MAGHRIB
  useEffect(() => {
    const fetchData = async () => {
      // Ambil nilai penyesuaian dari Supabase
      ...
      const adjValue = data?.nilai ? parseInt(data.nilai) : 0;
      setHijriAdj(adjValue);

      // Ambil Jadwal Maghrib Hari ini untuk Deteksi Pergantian Hijriyah
      try {
        const res = await fetch(
          `https://api.aladhan.com/v1/timingsByCity?city=Jakarta&country=Indonesia&method=20&adj=${adjValue}`,
        );
        ...
        checkMaghrib(); // Eksekusi saat pertama load
        const interval = setInterval(checkMaghrib, 60000); // Cek ulang setiap 1 menit
        return () => clearInterval(interval);
      } catch (error) {
        // Fallback jika API gagal: Anggap jam 18:00 adalah Maghrib
        const currentHour = new Date().getHours();
        setIsAfterMaghrib(currentHour >= 18);
      }
    };
    fetchData();
  }, []);
  ...
  const nextIslamicEvent = useMemo(() => {
    ...
    for (let i = 0; i < 365; i++) {
      const isToday = i === 0;
      // LOGIKA MAGHRIB: Jika ini hari ini dan sudah maghrib, tambah 1 hari untuk pengecekan Hijriyah
      const extraDay = isToday && isAfterMaghrib ? 1 : 0;
      const adjustedDate = new Date(
        checkDate.getTime() + (hijriAdj + extraDay) * 86400000,
      );
      ...
      if (event) {
        // isTonight true jika eventnya jatuh di hari ini (post-maghrib)
        return {
          ...event,
          dateObj: new Date(checkDate),
          isTonight: isToday && isAfterMaghrib,
        };
      }
      checkDate.setDate(checkDate.getDate() + 1);
    }
    return null;
  }, [hariBesarIslam, hijriFormatterParts, hijriAdj, isAfterMaghrib]);

  useEffect(() => {
    if (!nextIslamicEvent) return;

    // Jika eventnya sudah terjadi malam ini, langsung 00:00:00
    if (nextIslamicEvent.isTonight) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // LOGIKA MAGHRIB: Berikan +1 Hari Hijriyah ke kotak "Hari Ini" jika sudah Maghrib
    const extraDay = isToday && isAfterMaghrib ? 1 : 0;
    const adjustedDateObj = new Date(
      dateObj.getTime() + (hijriAdj + extraDay) * 86400000,
    );
    ...
    // Penanda UI jika kotak "Hari Ini" masuk Malam Pergantian Hijriyah
    const isPostMaghribCell = isToday && isAfterMaghrib;
    ...
    calendarCells.push(
      <div
        ...
        ${isPostMaghribCell && !isSelected ? 'bg-indigo-50/40 border-indigo-100' : ''}
      `}>
        {/* Ikon Bulan Penanda Malam Hijriyah */}
        {isPostMaghribCell && (
          <div
            className='absolute top-1.5 left-1.5'
            title='Malam pergantian Hijriyah'>
            <Moon className='w-3 h-3 text-indigo-400 fill-indigo-400/20' />
          </div>
        )}
        ...
        <span
          className={`text-[10px] md:text-xs font-semibold mt-0.5 ${isSelected ? 'text-mni-primary' : isPostMaghribCell ? 'text-indigo-500' : 'text-gray-400'}`}>
          {toArabic(hDay)}
        </span>
      </div>,
    );
  ...
  // LOGIKA TEKS KALENDER BESAR DI MODAL: Sesuaikan dengan isAfterMaghrib jika yang di-klik adalah hari ini
  const isSelectedToday =
    new Date().toDateString() === selectedDate.toDateString();
  const selectedExtraDay = isSelectedToday && isAfterMaghrib ? 1 : 0;

  const adjustedSelectedDate = new Date(
    selectedDate.getTime() + (hijriAdj + selectedExtraDay) * 86400000,
  );
  ...
  const agendaHariIni = useMemo(() => {
    ...
    const isTodayCell =
      new Date().toDateString() === selectedDate.toDateString();
    const extraDay = isTodayCell && isAfterMaghrib ? 1 : 0;
    const adjustedDate = new Date(
      selectedDate.getTime() + (hijriAdj + extraDay) * 86400000,
    );
  ...
  const eventsThisMonth = useMemo(() => {
    ...
    for (let d = 1; d <= maxDays; d++) {
      ...
      const isTodayCell = new Date().toDateString() === dateObj.toDateString();
      const extraDay = isTodayCell && isAfterMaghrib ? 1 : 0;
      const adjustedDateObj = new Date(
        dateObj.getTime() + (hijriAdj + extraDay) * 86400000,
      );
                ...
                <div className='relative z-10 flex flex-col items-center mb-2'>
                  <div className='text-center mb-5 mt-2'>
                    ...
                    <div className='inline-flex items-center justify-center bg-white/20 px-5 py-2 rounded-full border border-white/30 backdrop-blur-md shadow-inner'>
                      ...
                      {/* Bedge Tambahan Jika Malam Hijriyah */}
                      {isSelectedToday && isAfterMaghrib && (
                        <span className='ml-2.5 bg-indigo-500/80 text-white text-[9px] px-2 py-0.5 rounded-full border border-indigo-400/50 flex items-center shadow-sm'>
                          <Moon className='w-2.5 h-2.5 mr-1 fill-white/20' />{' '}
                          Malam
                        </span>
                      )}
                    </div>
