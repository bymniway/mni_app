...
// 1. IMPOR UTAS FIREBASE ADMIN SDK
...
export async function POST(request: Request) {
  try {
    ...
    // 2. KOMPRESI SHARP (Tetap dipertahankan)
    ...
    // Tambahan string acak agar nama file 100% unik dan tidak bentrok
    const fileName = `ziswaf_${Date.now()}_${Math.random().toString(36).substring(7)}.avif`;

    // 3. INISIALISASI SECURITY GATE FIREBASE ADMIN
    if (!getApps().length) {
      initializeApp({
        credential: cert({
    ...
    // 4. EKSEKUSI UNGGAH KE BUCKET RAHASIA FIREBASE
    const bucket = getStorage().bucket('mni-lan.firebasestorage.app');
    const targetPath = `ziswaf-bukti-transfer/${fileName}`;
    ...
    // 5. GENERATE AMBASSADOR KEY (SIGNED URL SAMPAI TAHUN 2499)
    const [signedUrl] = await firebaseFile.getSignedUrl({
      action: 'read',
      expires: '03-09-2499',
    });

    // 6. UPDATE DATABASE MENGGUNAKAN SIGNED URL FIREBASE
    const kodeTrx = `ZIS-${Date.now().toString().slice(-6)}`;
    await supabase.from('transaksi_ziswaf').insert([
      {
        kode_trx: kodeTrx,
        nama: nama,
        is_anonim: isAnonim,
    ...
    // Menambahkan logging agar lebih mudah dilacak jika terjadi eror
...
//
//
// komponen khusus cloudlflare r2 (tanpa supabase storage)
// import { NextResponse } from 'next/server';
// import { getServiceSupabase } from '@/lib/supabase';
// import sharp from 'sharp';
// import nodemailer from 'nodemailer';
// import { S3Client, PutObjectCommand } from '@aws-sdk/client-s3'; // 📦 Alat komunikasi R2

// export const dynamic = 'force-dynamic';
// export const revalidate = 0;

// // 🔧 Inisialisasi S3 Client khusus untuk R2 (Taruh di luar fungsi agar ringan)
...
//     // Supabase dipanggil HANYA untuk database SQL
//     const supabase = getServiceSupabase();

//     // 1. KOMPRESI FOTO (SHARP)
//     const arrayBuffer = await file.arrayBuffer();
//     const buffer = Buffer.from(arrayBuffer);
//     const avifBuffer = await sharp(buffer)
//       .avif({ quality: 50, effort: 4 })
...
//     // 📁 Simpan di virtual folder bukti_transfer/
//     const fileName = `bukti_transfer/ziswaf_${Date.now()}_${Math.random().toString(36).substring(7)}.avif`;
...
//         CacheControl: 'private, max-age=31536000', // Set private untuk keamanan bukti tf
//       }),
//     );
...
//         nominal: parseFloat(nominal),
//         rincian_kalkulasi: rincian || null,
//         bukti_transfer_url: finalUrl, // <-- Masukkan URL R2 ke Database
//         status_pesanan: 'Menunggu',
//       },
//     ]);
