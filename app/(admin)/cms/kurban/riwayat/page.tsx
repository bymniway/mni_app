...
export default function RiwayatPesananKurban() {
  ...
  const fetchRiwayat = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('pesanan')
        .select('*, hewan(jenis, tipe, berat, periode)')
  ...
  const handleSelectAllPeriode = () => {
    if (selectedPeriode.length === availablePeriode.length) {
      ...
      setSelectedPeriode([...availablePeriode]);
    }
  };
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    if (e.currentTarget.scrollTop > 500) setShowScrollTop(true);
  ...
  // 1. FILTER BERDASARKAN PERIODE TERLEBIH DAHULU (LOGIKA KOSONG JIKA UNCHECK SEMUA)
  const listFilteredByPeriode = useMemo(() => {
    if (selectedPeriode.length === 0) return []; // HANYA TAMPIL JIKA ADA PERIODE DIPILIH
    return pesananList.filter((p) =>
      selectedPeriode.includes(p.hewan?.periode || '1447 H'),
    );
  }, [pesananList, selectedPeriode]);

  // 2. KEMUDIAN FILTER BERDASARKAN JENIS & SEARCH
  const listFilteredByJenisAndSearch = useMemo(() => {
    return listFilteredByPeriode.filter((p) => {
      const matchJenis =
  ...
  }, [listFilteredByPeriode, filterJenis, searchTerm]);

  // 3. TERAKHIR FILTER BERDASARKAN STATUS
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, `Data_Kurban_MNI_${new Date().getTime()}.xlsx`);
  };
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(
      `Periode Kurban: ${selectedPeriode.length > 0 ? selectedPeriode.join(', ') : '-'}  |  Dicetak pada: ${new Date().toLocaleString('id-ID')}`,
      pageWidth / 2,
      55,
      { align: 'center' },
    );
    ...
    doc.save(`Daftar_Jagal_MNI_${new Date().getTime()}.pdf`);
  };
  ...
  const counts = useMemo(
    () => ({
      ...
      Terkirim: listFilteredByJenisAndSearch.filter(
  ...
  const metrics = useMemo(() => {
    const lunasData = listFilteredByJenisAndSearch.filter(
      (p) =>
        ...
        formatStatusDisplay(p.status_pesanan) === 'Terkirim',
    );
    const menungguData = listFilteredByJenisAndSearch.filter(
      (p) => formatStatusDisplay(p.status_pesanan) === 'Menunggu',
    );
    const terkirimData = listFilteredByJenisAndSearch.filter(
      (p) => formatStatusDisplay(p.status_pesanan) === 'Terkirim',
    );
    ...
    return {
      ...
      pendapatanAkhir: terkirimData.reduce(
        (sum, p) => sum + Number(p.total_bayar),
        0,
      ),
    };
      ...
      <div
        ...
        <div className='p-4 md:p-8 space-y-8'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5'>
            ...
            <div className='bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 group hover:border-emerald-300 hover:shadow-md hover:-translate-y-1 transition-all duration-300 relative overflow-hidden cursor-default'>
              <CheckCircle2 className='absolute -right-4 -bottom-4 w-24 h-24 text-emerald-50/60 group-hover:scale-110 transition-transform duration-700 pointer-events-none' />
              <div className='w-12 h-12 bg-slate-50 border border-slate-100 text-emerald-600 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-emerald-50 transition-colors relative z-10'>
              ...
              <div className='relative z-10 w-full overflow-hidden'>
                ...
                <div
                  ...
                  <motion.div
                    ...
                    className='flex w-max'>
                    <p className='text-xl font-bold text-emerald-600 pr-10'>
                      {formatRp(metrics.pendapatanAkhir)}
                    </p>
                    <p className='text-xl font-bold text-emerald-600 pr-10'>
                      {formatRp(metrics.pendapatanAkhir)}
                    </p>
          ...
          <div className='bg-white p-6 rounded-3xl shadow-sm border border-slate-200 h-[400px] flex flex-col mb-4'>
            ...
            {selectedPeriode.length === 0 ? (
              ...
              <div className='flex-1 flex items-center justify-center text-slate-400 text-sm font-medium'>
                Tidak ada data untuk ditampilkan pada periode terpilih
              </div>
            ) : (
          ...
          <div className='sticky top-0 z-40 -mx-4 px-4 md:-mx-8 md:px-8 py-4 bg-[#f8fafc]/90 backdrop-blur-xl'>
            <div className='bg-white p-3 md:p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col xl:flex-row gap-3 md:gap-4 justify-between items-center transition-all w-full'>
              ...
              {/* ACTION FILTERS: PERIODE, JENIS, SEARCH */}
              <div className='flex flex-row gap-3 w-full xl:w-auto shrink-0 z-50'>
                {/* 1. FILTER DROPDOWN CHECKBOX (SMART POPOVER) */}
                <div
                  className='relative w-max'
                  ref={periodeRef}>
                  <button
                    onClick={() => setIsPeriodeOpen(!isPeriodeOpen)}
                    className='flex items-center gap-2 px-3 sm:px-4 py-2.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 hover:border-teal-300 rounded-xl transition-all shadow-sm'>
                  ...
                  <AnimatePresence>
                    {isPeriodeOpen && (
                      <motion.div
                        ...
                        className='absolute left-0 xl:left-auto xl:right-0 top-full mt-2 w-48 bg-white border border-slate-200 rounded-2xl shadow-xl p-2 z-[60]'>
                        <button
                ...
                {/* 2. FILTER JENIS HEWAN */}
                <div className='relative w-1/3 xl:w-36'>
                  <Tag className='w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400' />
                  <select
                    value={filterJenis}
                    onChange={(e) => setFilterJenis(e.target.value)}
                    className='w-full pl-9 pr-8 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 text-xs font-bold text-slate-600 appearance-none cursor-pointer transition-all hover:bg-slate-100 shadow-sm'>
                ...
                <div className='relative flex-1 xl:w-64'>
                  ...
                  <input
                    type='text'
                    placeholder='Kode TRX / Nama...'
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className='w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 text-sm font-medium transition-all shadow-sm'
                  />
                </div>
              </div>
            </div>
              ...
              <table className='w-full text-left border-collapse min-w-[800px]'>
                <thead className='bg-slate-50/80 border-b border-slate-200'>
                  <tr className='text-slate-500 text-[11px] uppercase tracking-wider'>
                    ...
                    <th className='p-4 font-bold'>Paket & Periode</th>
                  ...
                  {isLoading ? (
                    ...
                    filteredPesanan.map((trx) => (
                      <tr
                        ...
                        <td className='p-4'>
                          <span className='text-xs font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200 mr-2'>
                        ...
                        <td className='p-4 text-center'>
                          <span
                            ...
                            ${formatStatusDisplay(trx.status_pesanan) === 'Lunas' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : formatStatusDisplay(trx.status_pesanan) === 'Selesai' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : formatStatusDisplay(trx.status_pesanan) === 'Terkirim' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : formatStatusDisplay(trx.status_pesanan) === 'Booking' ? 'bg-blue-50 text-blue-600 border-blue-200' : formatStatusDisplay(trx.status_pesanan) === 'Ditolak' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
        ...
        {selectedTrx && (
          <div className='fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'>
            <motion.div
              ...
              className='bg-white rounded-[2rem] w-full max-w-2xl my-8 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]'>
              <div className='p-5 border-b border-slate-100 bg-white flex justify-between items-center shrink-0'>
                ...
                <div className='text-right flex items-center gap-4'>
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-lg text-[11px] font-bold border ${formatStatusDisplay(selectedTrx.status_pesanan) === 'Lunas' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : formatStatusDisplay(selectedTrx.status_pesanan) === 'Selesai' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : formatStatusDisplay(selectedTrx.status_pesanan) === 'Terkirim' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : formatStatusDisplay(selectedTrx.status_pesanan) === 'Booking' ? 'bg-blue-50 text-blue-600 border-blue-200' : formatStatusDisplay(selectedTrx.status_pesanan) === 'Ditolak' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
...
//   const metrics = useMemo(() => {
//     // Pendapatan Lunas kini juga menyertakan yang sudah "Terkirim" agar grafiknya tidak turun
//     const lunasData = listFilteredByJenisAndSearch.filter(
//       (p) =>
//         formatStatusDisplay(p.status_pesanan) === 'Lunas' ||
//         formatStatusDisplay(p.status_pesanan) === 'Selesai' ||
...
//       (p) => formatStatusDisplay(p.status_pesanan) === 'Menunggu',
//     );
//     // Data Khusus untuk Pendapatan Akhir
//     const terkirimData = listFilteredByJenisAndSearch.filter(
//       (p) => formatStatusDisplay(p.status_pesanan) === 'Terkirim',
//     );
