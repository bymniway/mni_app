...
//   const isSelectionMode = selectedIds.length > 0; // Mode Seleksi Aktif
...
//           75% { transform: rotate(1deg) translate(0px, 0px); }
//           100% { transform: rotate(-1deg) translate(0px, 0px); }
//         }
//         .animate-jiggle {
//           animation: ios-jiggle 0.5s ease-in-out infinite;
//           transform-origin: center center;
//         }
//         /* Membuat goyangan lebih organik (tidak sinkron persis) antara ganjil & genap */
...
//             className='fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] bg-gray-900 text-white px-5 py-4 rounded-2xl shadow-2xl flex items-center gap-4 sm:gap-6 border border-gray-800 w-[90%] sm:w-auto overflow-x-auto whitespace-nowrap'>
//             <div className='flex items-center gap-3 pr-4 border-r border-gray-700'>
//               <span className='bg-mni-primary text-white text-xs font-bold px-3 py-1.5 rounded-lg'>
//                 {selectedIds.length} Terpilih
//               </span>
//               <button
//                 onClick={() =>
//                   setSelectedIds(hewanDitampilkan.map((h) => h.id))
//                 }
...
//                     className='flex items-center text-sm font-bold bg-gray-800 hover:bg-orange-600 px-4 py-2 rounded-xl transition-colors'>
//                     <Trash2 className='w-4 h-4 mr-2' /> Buang Massal
//                   </button>
//                   <button
//                     onClick={handleBulkHapusSembunyi}
//                     className='flex items-center text-sm font-bold bg-gray-800 hover:bg-red-600 px-4 py-2 rounded-xl transition-colors'>
//                     <EyeOff className='w-4 h-4 mr-2' /> Hapus Massal
//                   </button>
//                 </>
//               ) : (
//                 <>
...
//                     className='flex items-center text-sm font-bold bg-gray-800 hover:bg-emerald-600 px-4 py-2 rounded-xl transition-colors'>
//                     <ArchiveRestore className='w-4 h-4 mr-2' /> Pulihkan Massal
//                   </button>
//                   <button
//                     onClick={handleBulkHapusSembunyi}
//                     className='flex items-center text-sm font-bold bg-gray-800 hover:bg-red-600 px-4 py-2 rounded-xl transition-colors'>
...
//               className='ml-auto p-1.5 bg-gray-800 rounded-full hover:bg-red-500 transition-colors'>
//               <X className='w-4 h-4' />
//             </button>
//           </motion.div>
//         )}
...
//                 }}
//                 key={item.id}
//                 // ANIMASI JIGGLE DITERAPKAN DI SINI JIKA MODE SELEKSI AKTIF
//                 className={`bg-white rounded-3xl shadow-sm border overflow-hidden relative transition-all duration-300 group
//                 ${isSelectionMode ? 'animate-jiggle cursor-pointer' : 'hover:shadow-xl'}
//                 ${!isTersedia || activeTab === 'sampah' ? 'grayscale-[40%]' : ''}
//                 ${isSelected ? 'border-mni-primary ring-2 ring-mni-primary/20 shadow-md scale-[0.98]' : 'border-gray-100'}`}
//                 onClick={() => isSelectionMode && toggleSelect(item.id)} // Bisa klik dimana saja di card saat jiggle
//               >
...
//                   }}
//                   className='absolute top-4 left-4 z-20 bg-white/90 backdrop-blur-sm p-1.5 rounded-xl shadow-sm border border-gray-200 hover:bg-gray-50 transition-colors'>
//                   {isSelected ? (
//                     <CheckSquare className='w-5 h-5 text-mni-primary' />
//                   ) : (
//                     <Square className='w-5 h-5 text-gray-300' />
//                   )}
//                 </button>

//                 {/* Badges Status & Sampah */}
//                 <div className='absolute top-4 right-4 z-10 flex flex-col gap-2 items-end'>
//                   {activeTab === 'sampah' ? (
//                     <span className='px-3 py-1.5 rounded-full text-[10px] uppercase font-bold shadow-sm backdrop-blur-md border bg-red-100 text-red-700 border-red-200 flex items-center'>
//                       <Clock className='w-3 h-3 mr-1' /> Dihapus dlm {sisaHari}{' '}
//                       Hari
//                     </span>
//                   ) : (
//                     <>
//                       <span
//                         className={`px-3 py-1.5 rounded-full text-[10px] uppercase font-bold shadow-sm backdrop-blur-md border ${isTersedia ? 'bg-green-100/90 text-green-700 border-green-200' : 'bg-red-100/90 text-red-700 border-red-200'}`}>
//                         {item.status}
//                       </span>
//                       <span
//                         className={`px-3 py-1.5 rounded-full text-[10px] uppercase font-bold shadow-sm flex items-center backdrop-blur-md border ${item.mekanisme === 'Otomatis' ? 'bg-blue-50/90 text-blue-700 border-blue-200' : 'bg-orange-50/90 text-orange-700 border-orange-200'}`}>
...
//                 {/* Area Gambar */}
//                 <div className='h-52 w-full bg-gray-50 flex items-center justify-center relative overflow-hidden'>
//                   {item.gambar_url ? (
//                     <img
//                       src={item.gambar_url}
//                       alt={item.tipe}
...
//                     <span className='text-white text-xs font-bold uppercase tracking-wider bg-white/20 px-2 py-1 rounded backdrop-blur-sm'>
//                       {item.jenis}
//                     </span>
//                   </div>
//                 </div>
...
//                 {/* Detail & Tombol Aksi */}
//                 <div className='p-5'>
//                   <h3 className='text-xl font-bold text-mni-text mb-1 truncate'>
//                     {item.tipe || '-'}
//                   </h3>
//                   <p className='text-lg font-extrabold text-mni-primary mb-4'>
//                     Rp {Number(item.harga).toLocaleString('id-ID')}
//                   </p>

//                   <div className='flex justify-end gap-2 pt-4 border-t border-gray-100'>
//                     {/* Sembunyikan tombol aksi individual jika sedang mode Jiggle/Multi-select */}
//                     {!isSelectionMode &&
//                       (activeTab === 'katalog' ? (
//                         <>
//                           <button
...
//                               openEditModal(item);
//                             }}
//                             className='px-4 py-2 text-sm font-bold text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-600 hover:text-white transition-colors flex items-center'>
...
//                             }}
//                             className='px-4 py-2 text-sm font-bold text-orange-600 bg-orange-50 rounded-xl hover:bg-orange-600 hover:text-white transition-colors flex items-center'
...
//                             }}
//                             className='px-4 py-2 text-sm font-bold text-red-600 bg-red-50 rounded-xl hover:bg-red-600 hover:text-white transition-colors flex items-center'
//                             title='Hapus (Sembunyikan Total)'>
...
//                             }}
//                             className='px-4 py-2 text-sm font-bold text-emerald-600 bg-emerald-50 rounded-xl hover:bg-emerald-600 hover:text-white transition-colors flex items-center'>
...
//                             }}
//                             className='px-4 py-2 text-sm font-bold text-red-600 bg-red-50 rounded-xl hover:bg-red-600 hover:text-white transition-colors flex items-center'>
//                             <EyeOff className='w-4 h-4 mr-1.5' /> Hapus
//                           </button>
//                         </>
//                       ))}
//                   </div>
