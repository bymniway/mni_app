export const DAFTAR_HALAMAN_CMS = [
  { path: '/cms/beranda', label: 'Beranda' },
  { path: '/cms/kurban/verifikasi', label: 'Kurban - Verifikasi' },
  { path: '/cms/kurban/riwayat', label: 'Kurban - Riwayat' },
  { path: '/cms/kurban/katalog', label: 'Kurban - Katalog' },
  { path: '/cms/ziswaf/katalog', label: 'Ziswaf - Katalog' },
  { path: '/cms/ziswaf/riwayat', label: 'Ziswaf - Riwayat Donasi' }, // Centang yang ini saja nanti!
  { path: '/cms/ziswaf/verifikasi', label: 'Ziswaf - Verifikasi' },
  { label: 'Email Sistem', path: '/cms/email' },
  { label: 'Media Editor', path: '/cms/media' },
  { label: 'Moderasi Komentar', path: '/cms/komentar' },
  { label: 'Kurban - Distribusi', path: '/cms/kurban/distribusi' },
  // ... dan menu spesifik lainnya
];
