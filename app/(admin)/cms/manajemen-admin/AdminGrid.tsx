...
//         }
//         /* Biar goyangnya tidak seragam/kaku */
//         .jiggle-animation:nth-child(even) {
...
//           {isSelectMode ? (
//             <span className='text-mni-primary font-bold'>
//               {selectedIds.length} Admin Terpilih
//             </span>
//           ) : (
//             <span>Total: {admins.length} Admin</span>
//           )}
//         </div>
...
//               className='flex items-center px-4 py-2 bg-red-100 text-red-600 rounded-xl text-sm font-bold hover:bg-red-200 transition-colors disabled:opacity-50'>
...
//               setSelectedIds([]); // Reset pilihan kalau batal
//             }}
...
//               key={admin.id}
//               onClick={() => {
//                 // Hanya bisa diklik kalau mode pilih aktif & bukan akun kebal
...
//                 ${admin.is_root ? 'bg-gradient-to-br from-purple-50 to-indigo-50 border-purple-200' : 'bg-white border-gray-200'}
//                 ${isSelected ? 'ring-4 ring-red-400 border-red-400 shadow-xl' : !isSelectMode && 'hover:shadow-xl hover:-translate-y-1'}
//               `}>
//               {/* Ikon Background Transparan Mengembang */}
//               <div className='absolute -right-6 -bottom-6 opacity-[0.03] group-hover:opacity-10 group-hover:scale-125 transition-all duration-700 pointer-events-none'>
//                 {admin.is_root ? (
//                   <ShieldAlert className='w-48 h-48 text-purple-900' />
//                 ) : (
//                   <Shield className='w-48 h-48 text-gray-900' />
//                 )}
//               </div>

//               {/* Checkbox (Muncul saat Mode Select) */}
...
//               {/* Header Kartu (Email & Nama) */}
//               <div
//                 className={`relative z-10 flex justify-between items-start mb-5 transition-all ${isSelectMode && !isSuperAdmin ? 'pl-8' : ''}`}>
//                 <div>
...
//                 </div>
//                 {/* Sembunyikan tombol aksi individual jika sedang mode pilih massal */}
//                 {!isSelectMode && <AksiAdmin admin={admin} />}
//               </div>

//               {/* Badges Status & Role */}
//               <div className='relative z-10 flex flex-wrap gap-2 mb-5'>
//                 {admin.is_root ? (
//                   <span className='inline-flex items-center space-x-1 text-purple-700 bg-purple-100/80 backdrop-blur px-2.5 py-1 rounded-lg text-xs font-bold border border-purple-200'>
//                     <ShieldAlert className='w-3.5 h-3.5' />{' '}
//                     <span>ROOT DEWA</span>
//                   </span>
//                 ) : (
//                   <span className='inline-flex items-center space-x-1 text-blue-700 bg-blue-100/80 backdrop-blur px-2.5 py-1 rounded-lg text-xs font-bold border border-blue-200'>
//                     <Shield className='w-3.5 h-3.5' /> <span>ADMIN</span>
//                   </span>
//                 )}

//                 {admin.is_active ? (
//                   <span className='inline-flex items-center text-green-700 bg-green-100/80 px-2.5 py-1 rounded-lg text-xs font-bold border border-green-200'>
//                     <CheckCircle2 className='w-3.5 h-3.5 mr-1' /> Aktif
//                   </span>
//                 ) : (
//                   <span className='inline-flex items-center text-red-700 bg-red-100/80 px-2.5 py-1 rounded-lg text-xs font-bold border border-red-200'>
...
//               {/* Akses Halaman */}
//               <div className='relative z-10 bg-white/50 backdrop-blur-sm p-3 rounded-xl border border-gray-100/50'>
...
//                         <span
//                           key={path}
//                           className='bg-white border border-gray-200 text-gray-600 px-2 py-1 rounded shadow-sm text-[10px] font-medium'>
//                           {path.replace('/cms/', '')}
//                         </span>
//                       ))
//                     ) : (
